...
function showQuestion(){
    if(questions[currentQuestion]){
        ...
        let optionsHtml = ''
        for(let e in q.options){
            optionsHtml += `<div data-op="${e}"   class="option"><span>${parseInt(e) + 1}</span>${q.options[e]}</div>`
        }
...
function optionClick(element){
    let clickedOption = parseInt(element.target.getAttribute('data-op'))
